...
export const UserLoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isSignup) {
        try {
          await authApi.login({ email: formData.email, password: formData.password }); // Ensure we have a fresh token or session if needed, but basically just notify
          await emailApi.notify(formData.email, 'WELCOME_USER');
        } catch (e) {
          console.warn('Signup email notification failed', e);
        }
      }
      showToast(
