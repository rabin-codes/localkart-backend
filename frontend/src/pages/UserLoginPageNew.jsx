// src/pages/UserLoginPageNew.jsx
...
import { useAuth } from '../context/AuthContext'; // If you are using this

export const UserLoginPageNew = () => {
  const navigate = useNavigate();
  // const { login } = useAuth(); // Uncomment if you use AuthContext to manage state
  
  const [isSignup, setIsSignup] = useState(false);
  const [showAlert, setShowAlert] = useState(false); // Turned off demo alert by default
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 2. Handle Success
      // Assuming your backend returns { token: "..." } or similar
      ...
      setErrors({ form: error.message || "Failed to connect to the server." });
    } finally {
      setLoading(false);
    }
  };
