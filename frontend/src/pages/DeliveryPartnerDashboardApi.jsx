...
import { Truck, MapPin, Clock, DollarSign, AlertCircle, CheckCircle, LogOut, Navigation, Zap, Store } from 'lucide-react';
      ...
      <div className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white sticky top-0 z-40 shadow-lg">
        <div className="max-w-7xl mx-auto px-4 py-6 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-white rounded-full border-2 border-blue-400 overflow-hidden flex-shrink-0">
              <img 
                src="https://images.unsplash.com/photo-1544725176-7c40e5a71c5e?w=100&h=100&fit=crop" 
                alt="Partner Profile"
            ...
            <div className="lg:col-span-2">
              <Card className="bg-white p-6">
                ...
                <div className="space-y-4">
                  {(dashboard?.availableOrders || []).filter(o => o.status === 'Pending').map((order) => (
                    <div
                      key={order.id}
                      className="p-4 rounded-xl border-2 border-blue-200 bg-blue-50 hover:border-blue-400 transition"
                    >
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <p className="font-bold text-lg text-gray-900">{order.customerName || 'Customer'}</p>
                          <p className="text-sm text-gray-600 flex items-center gap-1">
                            <MapPin className="w-4 h-4" />
                            {order.deliveryAddress || order.address || 'Nearby order'}
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-2xl text-green-600">Rs. {order.total}</p>
                          <p className="text-xs text-gray-500">Earnings: Rs. {order.deliveryCharge || 40}</p>
                        </div>
                      </div>
                      <div className="flex justify-between items-center pt-3 border-t border-gray-200">
                        <div className="flex flex-col gap-1 mb-3">
                          {order.items?.map((item, i) => (
                            <div key={i} className="flex items-center gap-2 text-xs">
                              ...
                              <span className="font-semibold">{item.product?.vendor?.shopName || 'Local Shop'}:</span>
                        ...
                        <div className="flex items-center gap-2 text-gray-600">
                          ...
                          <span className="text-sm font-semibold">Ready for pickup from {order.items?.length || 0} shops</span>
                        </div>
                        <Button
                          variant="primary"
                  ...
                  {(dashboard?.availableOrders || []).filter(o => o.status === 'Pending').length === 0 && (
                    <p className="text-gray-500 text-center py-4">No available orders at the moment.</p>
                  )}
                </div>
              </Card>

              <Card className="bg-white p-6 mt-6">
                <h3 className="text-xl font-bold text-gray-900 mb-6">My Current Tasks</h3>
                <div className="space-y-4">
                  {(dashboard?.orders || []).filter(o => o.status === 'Accepted').map((order) => (
                    <div key={order.id} className="p-4 rounded-xl border-2 border-green-500 bg-green-50">
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <p className="font-bold text-lg text-gray-900">{order.customerName || 'Customer'}</p>
                          <p className="text-sm text-gray-600 flex items-center gap-1">
                            <MapPin className="w-4 h-4" />
                            {order.deliveryAddress || 'Ongoing delivery'}
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-xl text-green-600">Rs. {order.total}</p>
                        </div>
                      </div>
                      <Button
                        variant="primary"
                  ...
                  {(dashboard?.orders || []).filter(o => o.status === 'Accepted').length === 0 && (
